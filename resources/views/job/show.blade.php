{{-- <x-layout>
    ...
    <x-job-card :$job>
        ...
        @else
            <div class="text-center text-sm font-medium text-red-500">You have already applied for this job.</div>
        @endcan



    </x-job-card>
    <x-card class="mb-4 text-lg font-medium">
...
<x-layout>
    ...
    <x-card class="mb-4 text-lg font-medium">More jobs from the same company:</x-card>

    <x-card>
        <h2 class="mb-4 text-lg font-medium">{{ $job->employer->company_name }}</h2>
        <div class="text-sm text-slate-500">
            @foreach ($job->employer->jobs as $otherJob)
                <div class="mb-4 flex justify-between">
                    <div>
